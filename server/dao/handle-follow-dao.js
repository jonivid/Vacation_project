...
const getAllfollowedVacations = async (details) => {
    const sql = `select destination,v.id as vacationId,u.id as userId  from vacations v inner join users_followed_vacations ufv on  ufv.vacation_id=v.id  inner join users u on u.id = ufv.user_id where u.id=? and v.id=?`;
